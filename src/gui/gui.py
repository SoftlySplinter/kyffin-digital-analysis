...
class GUI:
    ...
    def render(self, data, data_type='default'):
        pass
...
class GraphGUI(GUI):
    def render(self, data):
        if self.data_type == 'histogram':
        ...
        elif self.data_type == 'hsv':
            self.renderGraph(data, ['Hue', 'Saturation', 'Value'], ['c-', 'm-', 'y-'])
        elif self.data_type == 'edge-orientation':
            ...
            raise Exception('Type "{0}" unknown.'.format(self.data_type.format(self.data_type)))
