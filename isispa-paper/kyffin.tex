...
\textbf{XXX Alex is going to combine the various correlation-against-K graphs so that we have one graph with all techniques on the same axes}

\begin{figure}[h]
\centering
\input{results/knn-mean/rgb}
\caption{Correlation Coefficients $r$ against K values for K-Nearest Neighbour on RGB Colour-Space Analysis}
\end{figure}

\begin{figure}[h]
\centering
\input{results/knn-mean/hsv}
...
%our framework. For each year represented in our collection we have asked a
%Kyffin Williams expert \textbf{ we could do with a bit more information
%here} to choose the one painting which best represents the artist's work for
%that year.  These we consider to be artistically chosen exemplars
%(\emph{artistic exemplars}, for short), which 
