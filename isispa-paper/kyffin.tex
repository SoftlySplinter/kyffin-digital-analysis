...
includes title, year or year ranges, genre, original painting size, painting materials and 
image size.
...
Within our database of X paintings, we know the actual year of painting for Y
...
with the other Y-1 paintings for which we know the date; we expect that there
...
measurement of colour space. We continued to use k-nearest neighbour, but with
different methods of calculating distance between each point in feature space;
namely chi squared.

%XXX I would concentrate on chi-squared, forget manhattan for this paper. 
%XXX basically all along we are generating histograms of feature response, 
%XXX and then using these to classify. the histogram might just mean mean 
%XXX R, G, B but it's a histogram all the same. 
...
number and strength of edges in the painting. 

%XXX which edge detection?  need a cite

Texture analysis is a continuation of edge detection. Instead of just taking
the strength and number of edges; it creates a histogram of orientated
gradients\cite{Dalal2005Histograms} begin to build up a more faithful
representation of the texture of a painting. To begin with steerable filters at
$0$, $\frac{\pi}{4}$, $\frac{\pi}{2}$ and $\frac{3\pi}{4}$ were applied to the
image to bin by those orientations.

%XXX can we have a scary equation here?
\begin{align}
...
\intertext{Where}
&(\omega_{x_0},\omega_{y_0}) \text{ Defines the centre frequency}\nonumber
\intertext{and}
&(\sigma_x,\sigma_y) \text{ the spread of the Gaussian window}\nonumber
\end{align}
...
%XXX can you draw a picture showing the influence of steerable filter angles?
...
%XXX I suggest for now doing results in-line, or have a table here showing 
%XXX correlation coefficients 
